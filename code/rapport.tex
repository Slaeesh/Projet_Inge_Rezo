...
La figure \ref{fig:exhaustive_benchmark} présente les résultats globaux sous forme d'une carte de chaleur (heatmap) des erreurs MAPE moyennes (moyennées sur les trois faisceaux) ainsi qu'un classement par nombre de configurations remportées (plus basse MAE).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{results/exhaustive_benchmark.png}
    \caption{Résultats globaux du benchmark exhaustif (Mode 6) : Heatmap des MAPE moyennes (\%) et barplot des victoires par modèle.}
