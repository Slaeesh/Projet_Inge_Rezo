...
Pour répondre à l'enjeu opérationnel d'identifier le meilleur algorithme prédictif selon chaque contexte, nous exploitons la recherche exhaustive stochastique (\textbf{Mode 6}). Cette dernière calcule les performances de tous les modèles sur les 36 configurations physiques résultant du produit cartésien : Voies (TX/RX) $\times$ Fréquences (1s/1min/10min) $\times$ Faisceaux (1/2/3) $\times$ Horizons (1/5), et ce sur l'ensemble des 6 scénarios de charge utilisateur (soit 1296 runs au total).

La figure \ref{fig:exhaustive_benchmark} présente les résultats globaux sous forme d'une carte de chaleur (heatmap) des scores $R^2$ moyens (moyennés sur les trois faisceaux et les six scénarios) ainsi qu'un classement par nombre de configurations remportées (plus basse MAE sur les 216 combinaisons scénario-physiques).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{results/exhaustive_benchmark.png}
    \caption{Résultats globaux du benchmark exhaustif (Mode 6) : Heatmap des $R^2$ moyens (tous faisceaux et scénarios confondus) et barplot des victoires par modèle.}
    \label{fig:exhaustive_benchmark}
\end{figure}

\subsubsection{Analyse Opérationnelle par Cas d'Usage et Justification des Métriques}
Les résultats du benchmark exhaustif (figure \ref{fig:exhaustive_benchmark}) mettent en lumière plusieurs comportements clés et permettent de dresser des règles de décision claires pour l'opérateur satellite.

\paragraph{Pourquoi les scores $R^2$ sont-ils faibles ou négatifs ?}
Une observation frappante du benchmark est la prédominance de scores $R^2$ négatifs ou proches de zéro, en particulier à haute fréquence (1s et 1min). Ce phénomène n'est pas le signe de prédictions aberrantes en pratique, mais s'explique par des facteurs mathématiques et méthodologiques stricts :
\begin{itemize}
    \item \textbf{Le paradoxe du $R^2$ sur des séries à faible variance} : Le coefficient $R^2 = 1 - \frac{MSE}{Var(Y_{test})}$ compare l'erreur quadratique à la variance du jeu de test. À l'agrégation 10min, le trafic est extrêmement stable et lissé, avec une variance de test minuscule. Dans ce cas, même avec une erreur de prédiction absolue extrêmement faible (une \textbf{MAPE} de seulement \textbf{2.6\%} pour le LSTM, soit $97.4\%$ de précision), l'erreur quadratique moyenne peut dépasser la variance naturelle de la série de test, rendant le $R^2$ négatif.
    \item \textbf{Le sous-apprentissage des réseaux récurrents (10 époques)} : Pour permettre l'exécution des 216 runs du benchmark en un temps raisonnable, le nombre d'époques d'apprentissage de GRU et LSTM a été bridé à 10 pour les résolutions 1s et 1min. C'est insuffisant pour que les poids convergent sur des jeux de données de taille importante. En revanche, à l'échelle 10min, la taille du dataset étant inférieure à 200 points, le programme force automatiquement 150 époques d'apprentissage, ce qui explique pourquoi GRU et LSTM y obtiennent des scores $R^2$ positifs.
    \item \textbf{Prédiction out-of-sample récursive des modèles statistiques} : Les modèles AR/MA et ARIMA effectuent des prédictions sur l'ensemble de test sans réajustement dynamique des valeurs passées réelles au fil de l'eau. Pour des horizons longs (plus de 200 pas de temps sur 1min), la prédiction converge inévitablement vers la moyenne historique, ce qui donne une ligne plate et un $R^2$ négatif face aux oscillations réelles.
    \item \textbf{La nature stochastique du trafic à la seconde} : Le trafic à la granularité de 1s (en particulier sur la voie retour RX) est intrinsèquement bruité et constitué de bouffées de Poisson sporadiques. Prédire le seconde-par-seconde est mathématiquement impossible avec de simples historiques (bruit blanc prédominant). De plus, la troncature à 1500 points pour la vitesse d'exécution prive les modèles de l'apprentissage du cycle quotidien de 24 heures.
\end{itemize}

\paragraph{Synthèse par cas d'usage :}
\begin{itemize}
    \item \textbf{Voie Aller (TX)} : Sur cette voie stable, les modèles de Deep Learning récurrents (\textbf{LSTM} et \textbf{GRU}) dominent dès que le nombre d'époques d'apprentissage est suffisant (10min). Ils apprennent la macro-dynamique temporelle avec une erreur MAPE très faible (environ 2.6\%).
    \item \textbf{Voie Retour (RX)} : Le trafic montant est marqué par de fortes variations instantanées. Face à ce bruit, ce sont les modèles robustes non paramétriques comme le \textbf{Random Forest} et la \textbf{SVR} qui s'imposent en MAE, évitant de sur-ajuster le bruit haute fréquence.
    \item \textbf{Impact de l'agrégation} : À 1s, le signal est trop instable. À 10min, le lissage permet d'utiliser des modèles statistiques simples et rapides (\textbf{AR/MA} et \textbf{ARIMA}) avec des erreurs absolues acceptables, bien que le Deep Learning conserve l'avantage en précision pure.
    \item \textbf{Impact de l'Horizon de Prédiction} : Lorsque l'on passe d'un horizon $H=1$ à un horizon $H=5$ pas dans le futur, la précision de tous les modèles décroît. C'est à cet horizon éloigné que l'écart entre le Deep Learning (qui conserve une mémoire à long terme) et les modèles autorégressifs simples s'accentue au profit du DL.
\end{itemize}
